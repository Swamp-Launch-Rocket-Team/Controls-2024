...
{
    this->drag_parameters.h = 4595 + z; // altitude above ground feet, first value is DEPENDENT on the launch site
	...
	this->drag_parameters.Servo_angle = (105) * U_airbrake;




    //Body Friction Drag Calc
    float Rn_star_body = (drag_parameters.a * drag_parameters.M * drag_parameters.L / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_body = 1 / (pow(1.89 + 1.62 * (log10(drag_parameters.L / k_body)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_f_body = Cf_final_body * (1 + (60 / (pow((drag_parameters.L / drag_parameters.d), 3))) + 0.0025 * (drag_parameters.L / drag_parameters.d)) * (4 * drag_parameters.Sb / (pi * pow(drag_parameters.d, 2)));
    ...
    float Rn_star_fin = (drag_parameters.a * drag_parameters.M * drag_parameters.Cr / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float k_fin = 0.0012;
	float Cf_star_term_fin = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Cr / k_fin)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Rn_fin = (drag_parameters.a * drag_parameters.M * drag_parameters.Cr) / (12 * visc); //Incompressible Re
	...
	float Cd_f_fins = Cf_lamda_fin * (1 + 60 * pow((drag_parameters.t / drag_parameters.Cr), 4) + 0.8 * (1 + 5 * pow((drag_parameters.Xtc / drag_parameters.Cr), 2)) * (drag_parameters.t / drag_parameters.Cr)) * (4.0 * (float)drag_parameters.Nf * drag_parameters.Sf) / (pi * pow(drag_parameters.d, 2));
    ...
    float Rn_star_railbutton1 = ((drag_parameters.a * drag_parameters.M * drag_parameters.Lp_railbutton1) / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_railbutton1 = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Lp_railbutton1 / k_railbutton1)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_pro_railbutton1 = Cf_pro_railbutton1 * (1 + 1.798 * (pow((sqrt(drag_parameters.A_railbutton1) / drag_parameters.Lp_railbutton1), 1.5))) * (4 * drag_parameters.Spro_rail_button) / (pi * pow(drag_parameters.d, 2));
	// Drag coefficient of rail button 1 protuberance

	float Rn_star_railbutton2 = ((drag_parameters.a * drag_parameters.M * drag_parameters.Lp_railbutton2) / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_railbutton2 = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Lp_railbutton2 / k_railbutton2)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_pro_railbutton2 = Cf_pro_railbutton2 * (1 + 1.798 * (pow((sqrt(drag_parameters.A_railbutton2) / drag_parameters.Lp_railbutton2), 1.5))) * (4 * drag_parameters.Spro_rail_button) / (pi * pow(drag_parameters.d, 2));
	// Drag coefficient of rail button 2 protuberance

		// same speed of sound a and viscosity visc as before
	float	Rn_star_bracketbase = ((drag_parameters.a * drag_parameters.M * drag_parameters.Lp_bracket) / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_bracketbase = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Lp_bracket / k_bracketbase)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_pro_bracketbase = 8 * Cf_pro_bracketbase * (1 + 1.798 * (pow((sqrt(drag_parameters.A_bracket_base) / drag_parameters.Lp_bracket), 1.5))) * (4 * drag_parameters.Spro_bracket_base) / (pi * pow(drag_parameters.d, 2));
	// Drag coefficient of 8 fin bracket bases

		// same speed of sound a and viscosity visc as before
	float	Rn_star_bracketheight = ((drag_parameters.a * drag_parameters.M * drag_parameters.Lp_bracket) / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_bracketheight = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Lp_bracket / k_bracketheight)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_pro_bracketheight = 8 * Cf_pro_bracketheight * (1 + 1.798 * (pow((sqrt(drag_parameters.A_bracket_height) / drag_parameters.Lp_bracket), 1.5))) * (4 * drag_parameters.Spro_bracket_height) / (pi * pow(drag_parameters.d, 2));
	...
	float Rn_star_airbrake = ((drag_parameters.a * drag_parameters.M * drag_parameters.Lp_airbrake) / (12 * visc)) * (1 + 0.0283 * drag_parameters.M - 0.043 * pow(drag_parameters.M, 2) + 0.2107 * pow(drag_parameters.M, 3) - 0.03829 * pow(drag_parameters.M, 4) + 0.002709 * pow(drag_parameters.M, 5));
	...
	float Cf_star_term_airbrake = 1 / (pow((1.89 + 1.62 * log10(drag_parameters.Lp_airbrake / k_airbrake)), 2.5));// Incompressible skin friction coefficient w / roughness
	...
	float Cd_pro_airbrake = Cf_pro_airbrake * (1 + 1.798 * (pow((sqrt(A_airbrake) / drag_parameters.Lp_airbrake), 1.5))) * (4 * Spro_airbrake) / (pi * pow(drag_parameters.d, 2));
	...
	float Cde = Ke * 4 * Sr_total / (pi * pow(drag_parameters.d, 2));// change in drag coeff due to excrescencies
    //cout << "Cde\t" << Cde << endl;
	...
	float Cd_press_fins = pow((1 - pow(drag_parameters.M, 2)), (-0.3)) - 1;// This was taken from the OpenRocket resource, adjusted for rectangular cross - section
